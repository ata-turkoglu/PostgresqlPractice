...
const create = (request, response) => {
  const { name, surname, depression, age, location, salary } = request.body;

  client.query(
    "INSERT INTO students (name, surname, depression, age, location, salary) VALUES ($1, $2, $3, $4, $5, $6)",
    [name, surname, depression, age, location, salary],
    (err, res) => {
      if (!err) {
...
const update = (request, response) => {
  ...
  client.query(
    "UPDATE students SET name = $1, surname= $2, age= $3, location= $4, depresion= $5,  salary= $6 WHERE name = $1",
    [name, surname, age, location, depresion, salary],
    (err, res) => {
      if (!err) {
...
const delByName = async (request, response) => {
  ...
  //promise
	/*
  client.query("DELETE FROM students WHERE name=$1", [name]).then((response) =>
    response
      .status(200)
  ...
  //async
	/*
  try {
    const response = await client.query("DELETE FROM students WHERE name=$1", [
      name,
    ]);
...
/*
pg paketi için
client.query("select * from name=$", [name])
bu kullanım da paket veriler sanitize  ediyor inject riski kalkıyor
